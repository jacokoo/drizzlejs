describe('Pageable Model', function() {
    ...
    it('should handle page turning correctly', function(done) {
        app.chain(app.start(), function() {
            ...
            expect(model.getPageInfo()).to.deep.equal({page: 1, start: 0, end: 0, total: 0});

            model.set({recordCount: 3, items: [{id: 1}, {id: 2}, {id: 3}]});
            expect(model.getPageInfo()).to.deep.equal({page: 1, start: 1, end: 2, total: 3});
            expect(model.getParams()).to.deep.equal({a: 1, _page: 1, pageSize: 2});

            model.turnToPage(2);
            expect(model.getPageInfo()).to.deep.equal({page: 2, start: 3, end: 3, total: 3});
    ...
    it('should use default values', function(done) {
        ...
        app.chain(app.start(), function() {
            ...
            expect(model.getPageInfo()).to.deep.equal({page: 5, start: 9, end: 10, total: 10});
