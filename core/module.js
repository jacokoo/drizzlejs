...
define(['jquery', 'underscore', 'backbone', './base', './view', './region'], function($, _, Backbone, Base, View, Region) {
  ...
  Module = (function(_super) {
    ...
    Module.prototype.loadData = function() {
      var collections, id, loadIt, models, promises, value,
        _this = this;
      this.data = {};
      promises = [];
      models = this.getOptionResult(this.options, 'models') || {};
      collections = this.getOptionResult(this.options, 'collections') || {};
      this.autoLoadDuringRender = [];
      this.autoLoadAfterRender = [];
      loadIt = function(id, value, isModel) {
        if (_this.data[id]) {
          throw new Error("data id: " + id + " is already used");
        }
        value = _this.getOptionResult(value);
        if (value) {
          (value.autoLoad === 'after' || value.autoLoad === 'afterRender' ? _this.autoLoadAfterRender : _this.autoLoadDuringRender).push(id);
        }
    ...
    Module.prototype.loadItems = function() {
      ...
      _fn = function(name, item) {
        ...
        p = _this.chain(_this.app.getLoader(name)[isModule ? 'loadModule' : 'loadView'](name, _this, item), function(obj) {
          ...
          if (item.region) {
            return _this.inRegionItems[item.region] = obj;
          }
        });
    ...
    Module.prototype.addRegion = function(name, el) {
      return this.regions[name] = new Region(this.app, this.module, name, el);
    };

    Module.prototype.render = function() {
      if (!this.region) {
        return this.logger.error('No region to render in');
      }
    ...
    Module.prototype.close = function() {
      ...
      return this.chain("Close module: " + this.name, (_ref1 = this.options.beforeClose) != null ? _ref1.apply(this) : void 0, (function() {
        var _ref2, _results;
        _ref2 = this.regions;
        _results = [];
        for (key in _ref2) {
        ...
        return _results;
      }).call(this), this.layout.close(), (_ref2 = this.options.afterClose) != null ? _ref2.apply(this) : void 0);
    };
